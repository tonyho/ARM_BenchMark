/*
 * lat_fs_chown.c - perform chown in a loop )
 *
 * Usage: lat_fs_chown [-C] [-P <parallelism] [-W <warmup>] [-N <repetitions>] file
 *
 */
char	*id = "$Id$\n";
...
initialize(iter_t iterations, void* cookie)
{
	...
	if (state->lock_uid) {
		...
		if (state->use_uid) {
			...
			ret |= get_quota_n(state->filename,
					benchmp_childid_uid() + 1,
					benchmp_childid_gid() + 1,
					state->qfd[1], state->lock_uid);
		} else {
			ret |= get_quota_n(state->filename,
					geteuid(), getegid(),
					state->qfd[0], state->lock_uid);
	...
	if (fd < 0)
		DO_ABORT("open");
	if (state->use_uid) {
		ret = fchown(fd, benchmp_childid_uid(),
			benchmp_childid_gid());
		if (ret)
			DO_ABORT("chown");
	}
...
void
time_with_chown(iter_t iterations, void * cookie)
{
	state_t	*state = (state_t *) cookie;
	...
	while (iterations-- > 0) {
		ret |= fchown(fd, benchmp_childid_uid(),
			benchmp_childid_gid());
		ret |= fchown(fd, benchmp_childid_uid() + 1,
			benchmp_childid_gid() + 1);
	}
	if (ret)
		DO_ABORT("error while test");
}
...
main(int ac, char **av)
{
	...
	benchmp(initialize, time_with_chown, cleanup,
