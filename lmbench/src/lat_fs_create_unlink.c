/*
 * lat_create_unlink.c - creat/unlink)
 *
 * Usage: lat_create_unlink [-C] [-P <parallelism] [-W <warmup>] [-N <repetitions>] file
 *
 */
char	*id = "$Id$\n";
...
void
time_with_create_unlink(iter_t iterations, void * cookie)
{
	state_t	*state = (state_t *) cookie;
	...
	int ret = 0;
	while (iterations-- > 0) {
		fd = open(filename, O_RDONLY|O_CREAT| rw, 0666);
		ret |= close(fd);
		unlink(filename);
	}
	if (ret)
...
main(int ac, char **av)
{
	...
	benchmp(initialize, time_with_create_unlink, cleanup,
