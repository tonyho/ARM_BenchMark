/*
 * lat_fs_write_truncate - write/truncate in a loop )
 *
 * Usage: lat_write_truncate [-C] [-P <parallelism] [-W <warmup>] [-N <repetitions>] file
 *
 */
char	*id = "$Id$\n";
...
void
time_with_loop(iter_t iterations, void * cookie)
{
	state_t	*state = (state_t *) cookie;
	...
	char buf [40960];
	while (iterations-- > 0) {
		ret |= (pwrite(fd, buf, 4096, 40960) != 4096);
